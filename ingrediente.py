...
class ingrediente:
    ...
    def listar_ingredientes():
        ...
        OracleConnection = oracleConnection()
        OracleConnection.cursor.execute('SELECT IngredienteID, Nome, Alergenico, UnidadeID FROM ingrediente WHERE (Excluido = 0 or Excluido = NULL) and dataExclusao > :1'(dataHoje))
    ...
    def excluir_ingrediente():
        ...
        try:
            OracleConnection = oracleConnection()
            OracleConnection.cursor.execute('Update ingrediente SET Excluido = :1, dataExclusao = :2  where ingredienteID = :3'(1, dataHoje, ingredienteID))
